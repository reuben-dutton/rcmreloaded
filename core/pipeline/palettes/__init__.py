...
class GradientPalette(_PaletteBase):
    ...
    def _reshape(self, input: list[Colour]) -> list[Colour]:
        ...
        output = [
            colours.make_hex(i(x / (n-1)).convert('srgb').to_string(hex=True))
            for x in range(n)
        ]
        return output
...
def _rotated(base: "coloraide.Color", degrees: float) -> Colour:
    ...
    return colours.make_hex(rotated.to_string(hex=True))
...
class AccentPalette(_PaletteBase):
    ...
    def _reshape(self, input: list[Colour]) -> list[Colour]:
        ...
        for hue_shift, light_shift in ((-25, 0.12), (0, 0), (25, -0.12)):
            ...
            output.append(colours.make_hex(muted.convert('srgb').to_string(hex=True)))

        accent = base.clone().set('c', max(base['c'], self.ACCENT_CHROMA))
        output.append(colours.make_hex(accent.convert('srgb').to_string(hex=True)))
        return output
    ...
    def _reshape(self, input: list[Colour]) -> list[Colour]:
        ...
        for i in range(n):
            ...
            if position < 0:
                ...
                mixed = base.clone()
            output.append(colours.make_hex(mixed.convert('srgb').to_string(hex=True)))
        return output
